...
async function updateTransactionStatus(txnRef, status, vnpayData = null) {
  try {
    // Check if the transaction exists
    const { data: existingTransaction, error: fetchError } = await supabase.from('transactions').select('id, status, order_id').eq('order_id', txnRef).single();
    if (fetchError) {
    ...
    const { data, error } = await supabase.from('transactions').update(updateData).eq('order_id', txnRef).select();
    if (error) {
...
async function updatePendingTransactionToSuccess(txnRef, vnpayData) {
  try {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { vnp_Amount, vnp_BankCode, vnp_BankTranNo, vnp_CardType, vnp_OrderInfo, vnp_PayDate, vnp_ResponseCode, vnp_TmnCode, vnp_TransactionNo, vnp_TransactionStatus, vnp_TxnRef, txnRef, vnp_SecureHash, ...otherParams } = params;
    ...
    if (!transactionRef || !vnp_SecureHash || vnp_TmnCode !== config.vnp_TmnCode) {
      await logMessage({
      ...
      return new Response(JSON.stringify({
        status: 'error',
        message: 'Missing or invalid required parameters (transactionRef, secure hash, or TmnCode)',
        transactionRef: transactionRef || 'undefined'
      }), {
        status: 400,
        headers: {
    ...
    const calculatedSignature = await generateVNPaySignature(signData, config.vnp_HashSecret);
    // Verify signature
    ...
    if (vnp_ResponseCode === '00' && vnp_TransactionStatus === '00') {
      ...
      const errorMessage = errorMessages[vnp_ResponseCode] || 'Giao dịch không thành công do lỗi không xác định.';
      return new Response(JSON.stringify({
        status: 'failed',
