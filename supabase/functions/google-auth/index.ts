...
serve(async (req)=>{
  try {
    ...
    const existingUser = existingUsers.users.find((user)=>user.email === verifiedUserInfo.email);
    if (existingUser) {
      ...
      // Get user profile
      const { data: patientProfile, error: profileError } = await supabase.from('patients').select('*').eq('user_id', existingUser.id).single();
      return new Response(JSON.stringify({
        success: true,
      ...
      const { data: patientProfile, error: profileError } = await supabase.from('patients').insert({
