...
serve(async (req) => {
  ...
  const { data: blogData, error: blogError } = await supabase.from("blog_posts")
    .select(`
    blog_id,
    blog_title,
    blog_content,
  ...
  const response = await Promise.all(blogData.map(async (blog) => {
    let imageUrl = null;
    if (blog?.image_link) {
      const { data: publicData } = await supabase.storage.from("blog-uploads")
        .getPublicUrl(blog.image_link);
